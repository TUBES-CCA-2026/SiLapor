...
<main class="w-full min-w-0 px-4 py-6 md:px-8 md:py-8 space-y-6">
    <header class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2">
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-lg sm:text-xl md:text-2xl font-extrabold text-[#2C3E50] tracking-tight uppercase">RIWAYAT</h1>
        </div>
...
<section class="page-card">
    <div class="page-card-body">
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
            <div>
                <h2 class="section-title" style="margin-bottom: .35rem;">Riwayat Laporan Selesai</h2>
                <p style="margin: 0; color: #64748B; font-size: .9rem;">Daftar laporan yang sudah selesai ditangani.</p>
            </div>
        </div>
            ...
            <table class="report-table">
                ...
                <tbody>
                    @forelse($riwayat as $item)
                        ...
                        @endphp
                        <tr>
                            <td style="font-weight: 800; color: #64748B;">PGD-{{ str_pad((string) $item->id_pengaduan, 3, '0', STR_PAD_LEFT) }}</td>
                            <td>{{ $item->fasilitas?->laboratorium?->nama_laboratorium ?? '-' }}</td>
                    ...
                    @empty
                        <tr><td colspan="6" class="empty-state">Belum ada riwayat pengaduan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
