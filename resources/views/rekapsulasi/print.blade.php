...
@foreach($rows as $row)<tr><td>{{ $row->tanggal_lapor ? $row->tanggal_lapor->format('d/m/Y') : '-' }}</td><td>{{ $row->pelapor?->nama ?? '-' }}</td><td>{{ $row->fasilitas?->nama_fasilitas ?? '-' }}</td><td>{{ $row->fasilitas?->laboratorium?->nama_laboratorium ?? '-' }}</td><td>{{ $row->status_pengaduan }}</td><td>{{ $row->deskripsi_kerusakan }}</td></tr>@endforeach
